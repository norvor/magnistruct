<script lang="ts">
    ...
    // Helper to safely get classic data
    $: c = task.classic;
</script>

<div 
    class="task-card {c?.priority || 'p4'}" 
    draggable={true}
...
>
    <div class="card-top">
        ...
        {#if c?.assignee}
            <div class="avatar" title={c.assignee.full_name}>
                {c.assignee.full_name[0].toUpperCase()}
            </div>
        {/if}
    </div>
    ...
    <div class="card-footer">
        <div class="tag priority-tag">
            {(c?.priority || 'p4').replace('p', 'P')}
        </div>

        {#if c?.due_date}
            <div class="tag date-tag">
                📅 {new Date(c.due_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
            </div>
        {/if}
    </div>
</div>
